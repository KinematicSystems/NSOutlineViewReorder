...
extension ViewController: NSOutlineViewDataSource, NSPasteboardItemDataProvider {
    ...
    func outlineView(_ outlineView: NSOutlineView, acceptDrop info: NSDraggingInfo, item: Any?, childIndex index: Int) -> Bool {
        ...
        debugPrint(
            "move src:\(srcItem.name) " +
            "dest:\(String(describing: destItem?.name)) " +
            "destIndex:\(index) oldIndex:\(oldIndex) " +
            "srcParent:\(String(describing: parentItem?.name)) " +
            "toIndex:\(toIndex) toParent:\(String(describing: destItem?.name)) " +
            "childIndex:\(index)", terminator: "")
